...
class CountingContextFreeGrammar:
    ...
    def __init__(
        ...
        for i, rule_string in enumerate(productions):
            ...
            for rhs in rhss.split("|"):
                tokenize = CountingContextFreeGrammar._tokenize # 클래스에 소속되었을 뿐 self (instance(객체체) 상태)에 의존하지 않음음
                production = [tokenize(e) for e in rhs.split()] # 그냥 split함수는 다 쪼갬 
    ...
    def generate(self, *, degree: int = 0) -> str:
        ...
        while derivation_queue:
            ...
            if token_type == TokenType.TERMINAL:
                ...
                string_list.append(
                    self._sample_terminal(terminal, assignment, degree) # 시간을 많이 쓰는 함수 
                )
                ...
                if self._is_counter(variable):
                    ...
                    string_list.append(" ".join(production_))
    ...
    def _sample_terminal(
        ...
        counter_operands = list(self._parse_counter_oparands(match.group(1)))
        ...
        terminal_list = random.choices(counter_operands,k=terminal_len)
